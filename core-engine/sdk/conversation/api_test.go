...
func TestConversationAPIs(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "GET /api/v1/conversations/stats":
		...
		case "DELETE /api/v1/conversations/c1", "POST /api/v1/conversations/c1/turn/cancel":
			writeJSON(t, w, success(map[string]any{}))
		case "GET /api/v1/conversations/c1/workspace/files":
	...
	sent, err := client.SendMessage(ctx, SendMessageRequest{ConversationID: "c1", GroupID: "g1", Content: " hello "})
	require.NoError(t, err)
	require.Equal(t, "r1", sent.Run.ID)
...
func TestStreamEvents(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.NoError(t, err)
		eventData, err := json.Marshal(Event{ID: 8, Type: "item.completed", ConversationID: "c1", RunID: "r1"})
		require.NoError(t, err)
