...
func NewUsageDetails(ctx context.Context, svcCtx *svc.ServiceContext, r *http.Request) *UsageDetails {
	return &UsageDetails{
...
func (l *UsageDetails) UsageDetails(req *types.TokenUsageDetailsRequest) (resp *types.TokenUsageDetailsResponse, err error) {
...
func normalizeUsagePage(current, size int) (int, int) {
	if current < 1 {
		current = 1
	}
	if size < 1 {
