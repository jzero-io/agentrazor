...
import { useWorkspacePanel, type FilePreview, type WorkspaceDescriptor } from './hooks/business/useWorkspacePanel';
...
import ConversationSidebar from './layouts/modules/conversation-sidebar/index.vue';
import LoginModal from './layouts/modules/login-modal/index.vue';
import RightPanel from './layouts/modules/right-panel/index.vue';
import ConversationView from './views/conversation/index.vue';
...
function normalizeWorkspaceFilePath(href: string) {
  return workspacePanel?.normalizeFilePath(href) || '';
}
...
function openWorkspace(workspace: WorkspaceDescriptor) {
  ...
  workspacePanel.openWorkspace(workspace);
}

async function openWorkspaceFile(path: string) {
  pinnedSummaryOpen.value = false;
  await workspacePanel.openFile(path);
}

function closeFilePreview() {
  workspacePanel.closeFile();
}

function displayWorkspaceFilePath(file: FilePreview) {
  return workspacePanel.displayWorkspaceFilePath(file);
}

function displayWorkspaceProcessPath(filePath: string) {
  return workspacePanel.displayWorkspaceProcessPath(filePath);
}
...
function toggleRightPanel() {
  ...
  if (activeWorkspace.value || activeFilePreview.value) {
    pinnedSummaryOpen.value = false;
...
function toggleWorkspaceExpanded() {
  workspacePanel.toggleExpanded();
}

function reloadWorkspace() {
  workspacePanel.reload();
}
...
const rightPanelContentReady = computed(() => Boolean(activeWorkspace.value || activeFilePreview.value || filePreviewLoading.value || filePreviewError.value));
