...
export function useConversationStreamEvents(options: ConversationStreamEventsOptions) {
  ...
  function parseStreamPosition(value?: string) {
  ...
  function eventFollowsSnapshot(event: StreamEvent, snapshotStreamPosition?: string) {
    if (!snapshotStreamPosition) return true;
    const snapshot = parseStreamPosition(snapshotStreamPosition);
    const current = parseStreamPosition(event.streamPosition);
    if (!snapshot || !current) return false;
    if (current.epoch !== snapshot.epoch) return false;
    return current.sequence > snapshot.sequence;
  }

  async function handleBufferedStreamEvents(events: StreamEvent[], snapshotStreamPosition?: string) {
    const conversations = new Set<string>();
    for (const event of events) {
      if (!eventFollowsSnapshot(event, snapshotStreamPosition)) continue;
